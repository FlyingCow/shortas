...
use salvo::core::http::header::{HeaderMap, HeaderName, HeaderValue, CONNECTION, HOST, UPGRADE};
use salvo::core::http::uri::Uri;
use salvo::core::http::{ReqBody, ResBody, StatusCode};
use salvo::core::{async_trait, BoxedError, Depot, Error, FlowCtrl, Handler, Request, Response};

pub use hyper_client::*;

use super::hyper_client;
        ...
        match self.build_proxied_request(req, depot).await {
            Ok(proxied_request) => {
                    ...
                    .await
                {
                    Ok(response) => {
                        let (
                            salvo::core::http::response::Parts {
                                status,
                                // version,
                                headers,
                                // extensions,
