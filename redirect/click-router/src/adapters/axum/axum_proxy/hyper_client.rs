...
use super::{Client, HyperRequest, HyperResponse, Proxy, Upstreams};
...
pub struct HyperClient {
    inner: HyperUtilClient<HttpsConnector<HttpConnector>, Body>,
}
...
impl HyperClient {
    ...
    pub fn new(inner: HyperUtilClient<HttpsConnector<HttpConnector>, Body>) -> Self {
        Self { inner }
    }
}
...
impl Client for HyperClient {
    ...
    async fn execute(
        ...
        let request_upgrade_type =
            super::get_upgrade_type(proxied_request.headers()).map(|s| s.to_owned());

        let mut response = self
            .inner
            .request(proxied_request)
            .await
            .map_err(|e| super::ProxyError::from(format!("Proxy request failed: {}", e)))?;

        if response.status() == StatusCode::SWITCHING_PROTOCOLS {
            let response_upgrade_type = super::get_upgrade_type(response.headers());
            if request_upgrade_type == response_upgrade_type.map(|s| s.to_lowercase()) {
                let response_upgraded = hyper::upgrade::on(&mut response)
                    .await
                    .map_err(|e| super::ProxyError::from(format!("Upgrade failed: {}", e)))?;
                ...
                if let Some(request_upgraded) = request_upgraded {
                    ...
                    return Err(super::ProxyError::from("request does not have an upgrade extension."));
                }
            } else {
                return Err(super::ProxyError::from("upgrade type mismatch"));
            }
        }

        // Convert the response body from Incoming to Body
...
mod tests {
    ...
    async fn test_upstreams_elect() {
        let _result = rustls::crypto::CryptoProvider::install_default(rustls::crypto::ring::default_provider());
