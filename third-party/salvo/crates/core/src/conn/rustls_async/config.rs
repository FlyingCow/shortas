...
impl RustlsConfig {
    ...
    /// ServerConfig
    pub fn build_server_config(mut self) -> IoResult<ServerConfig> {
        let fallback = self
            .fallback
            .as_mut()
...
///Rustls configuration resolver
#[async_trait::async_trait()]
pub trait ResolvesServerConfig<E>
where E: std::error::Error + Send {
    ///Resolves server configuration based on client hello info
    async fn resolve(&self, client_hello: ClientHello<'_>) -> Result<Arc<RustlsConfig>, E>;
}
