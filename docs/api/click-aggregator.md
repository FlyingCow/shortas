---
layout: page
title: Click Aggregator API
permalink: /api/click-aggregator/
---
...
# JWT Configuration
KEYCLOAK_URL=http://keycloak:8080
KEYCLOAK_REALM=click-aggregator
KEYCLOAK_CLIENT_ID=click-aggregator-api
...
1. **Build Docker Image**
   ```bash
   docker build -t click-aggregator-api .
   ```

2. **Run Container**
   ```bash
   docker run -p 8080:8080 \
     -e DATABASE_URL=mongodb://mongodb:27017/click_aggregator \
...
services:
  api:
    ...
    ports:
      - "8080:8080"
    environment:
      - DATABASE_URL=mongodb://mongodb:27017/click_aggregator
      - KEYCLOAK_URL=http://keycloak:8080
    depends_on:
      - mongodb
      - keycloak
...
1. **Environment Setup**
   ```bash
   export DATABASE_URL="mongodb://production-db:27017/click_aggregator"
...
2. **Run Application**
   ```bash
   ./target/release/click-aggregator-api
   ```
